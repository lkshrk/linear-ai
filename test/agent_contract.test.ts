...
test("implementer requires permission context for direct and orchestrated runs", async () => {
  const implementerAgent = await readAgent("implementer");
  const implementerDoc = await readDoc("docs/implementer.md");
  const implementSkill = await readDoc("skills/linear-implement/SKILL.md");
  const orchestratorAgent = await readAgent("orchestrator");
  const orchestratorDoc = await readDoc("docs/orchestrator.md");
  const batchImplementSkill = await readDoc("skills/linear-batch-implement/SKILL.md");
  const deliverFeatureSkill = await readDoc("skills/linear-deliver-feature/SKILL.md");

  for (const source of [implementerAgent, implementerDoc, implementSkill]) {
    assert.match(source, /required implementer permission context/i);
    assert.match(source, /workspace write access/i);
    assert.match(source, /package manager/i);
    assert.match(source, /verification command|verification commands/i);
    assert.match(source, /Linear MCP read\/write tools/i);
    assert.match(source, /Do not begin routine implementation in a prompt-by-prompt permission mode/i);
  }

  for (const source of [orchestratorAgent, orchestratorDoc, batchImplementSkill, deliverFeatureSkill]) {
    assert.match(source, /same required implementer permission context used by direct/i);
    assert.match(source, /workspace write access/i);
    assert.match(source, /package manager/i);
    assert.match(source, /verification command/i);
    assert.match(source, /Linear MCP read\/write tools/i);
  }

  for (const source of [implementerAgent, implementerDoc, implementSkill, batchImplementSkill]) {
    assert.match(source, /destructive, irreversible, credential-gated, external-production/i);
    assert.match(source, /materially scope-changing/i);
    assert.match(source, /missing (required )?authority|missing-authority/i);
  }
});

test("implementer uses an isolated issue worktree as the primary workspace", async () => {
  const implementerAgent = await readAgent("implementer");
  const implementerDoc = await readDoc("docs/implementer.md");
  const implementSkill = await readDoc("skills/linear-implement/SKILL.md");
...
test("implementer distinguishes issue worktree cleanup from temporary lane worktrees", async () => {
  const implementerAgent = await readAgent("implementer");
  const implementerDoc = await readDoc("docs/implementer.md");
  const implementSkill = await readDoc("skills/linear-implement/SKILL.md");
  ...
  for (const source of [implementerAgent, implementerDoc, implementSkill]) {
    ...
    assert.match(source, /issue worktree/i);
    assert.match(source, /temporary lane worktrees/i);
...
test("plan template separates issue worktree from Git ref plumbing", async () => {
  const template = await readDoc("templates/linear-plan-comment.md");

  assert.match(template, /Issue worktree:/);
  assert.match(template, /Git ref:/);
  assert.match(template, /feature branch with PR/i);
  assert.doesNotMatch(template, /Branch: `replace-with-branch-name`/);
});
...
test("workflow skills define step completion handoff instead of stopping silently", async () => {
  ...
  const docs = [...standaloneDocs, "skills/linear-deliver-feature/SKILL.md"];
  ...
  for (const docPath of standaloneDocs) {
    const doc = await readDoc(docPath);
    assert.match(doc, /Question: Is there anything else to add before moving on\?/i, `${docPath} should preserve the add-more question`);
    assert.match(
      doc,
      /Question: Do you want to continue with the recommended next skill\?/i,
      `${docPath} should ask whether to continue with the recommended next skill`
    );
    assert.match(doc, /do not auto-run|without user confirmation/i, `${docPath} should not auto-run the next skill`);
  }
});
