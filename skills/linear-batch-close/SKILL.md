...
Immediately before dispatching each issue, re-read issue state with `get_issue` and `list_comments`. Skip the issue if it no longer has `llm-review`, and record observed Linear state drift in the summary.
