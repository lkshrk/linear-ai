...
- Before marking a ready plan, decide from source evidence whether material ambiguity remains. Source evidence includes the issue, comments, linked docs, and codebase inspection. If no material ambiguity remains, record that no questionnaire was needed.
...
- When material ambiguity remains, start a real questionnaire/interview path. Use `grill-me` or `grill-with-docs` when available; otherwise run the fallback directly by asking exactly one concrete question, providing the recommended answer, and waiting for the user's response before continuing.
- Run the Mandatory Local Grill Pass from `docs/agent-required-passes.md` when external grill skills are unavailable.
- Mandatory Local Grill Pass: challenge hidden ambiguity before marking ready.
- Do not set `plan_status: ready` until the grill pass has resolved every material branch or recorded accepted unknowns, and the plan records either that no questionnaire was needed or that questionnaire answers/accepted unknowns resolved the ambiguity.
...
- When the refinement step is complete, first ask whether there is anything else to add for refinement. If the answer is no, ask whether the user wants to continue with the recommended next skill, normally `linear-implement` when `llm-ready` is present. Name the skill and wait for confirmation; do not auto-run it.
