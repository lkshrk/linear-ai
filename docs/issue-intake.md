...
At the completed intake handoff, ask whether there is anything else to add for intake. If the answer is no, ask whether the user wants to continue with the recommended next skill, normally `linear-refine` when `llm-refine` is present. Name the recommended skill and wait for confirmation; do not auto-run it.
